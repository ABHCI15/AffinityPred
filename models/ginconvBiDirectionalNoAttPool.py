...
class AdaptivePooling(nn.Module):
    """Adaptive pooling mechanism for graph representations with edge features."""
    def __init__(self, input_dim, edge_dim, hidden_dim=256):
        super().__init__()
        # We concatenate mean/max/sum pooled features -> 3x input size
        self.node_pool_proj = nn.Linear(input_dim * 3, hidden_dim)
        self.edge_pool_proj = nn.Linear(edge_dim * 3, hidden_dim)
        
    def forward(self, g, node_feat, edge_feat):
        # Node pooling
        g.ndata['h'] = node_feat
        node_mean = dgl.mean_nodes(g, 'h')
        node_max = dgl.max_nodes(g, 'h')
        node_sum = dgl.sum_nodes(g, 'h')
        
        # Edge pooling
        g.edata['e'] = edge_feat
        edge_mean = dgl.mean_edges(g, 'e')
        edge_max = dgl.max_edges(g, 'e')
        edge_sum = dgl.sum_edges(g, 'e')
        
        # Project to common dimension
        node_combined = torch.cat([node_mean, node_max, node_sum], dim=1)
        edge_combined = torch.cat([edge_mean, edge_max, edge_sum], dim=1)
        
        node_proj = self.node_pool_proj(node_combined)
        edge_proj = self.edge_pool_proj(edge_combined)
        
        # Combine node and edge features
        return torch.cat([node_proj, edge_proj], dim=1)
...
class GINWithBidirectionalAttention(nn.Module):
    ...
    def __init__(self, node_feat_dim, edge_feat_dim, h_feats=256, prot_dim=1152, 
        ...
        # Adaptive pooling mechanism (updated for edge features)
        self.adaptive_pool = AdaptivePooling(h_feats, edge_feat_dim, hidden_dim=h_feats//2)
        
        # Molecule projection with skip connections (updated for edge-aware pooling)
        self.mol_proj = nn.Sequential(
            nn.Linear(h_feats, emb_dim * 2),  # Node + edge pooled features
            nn.GELU(),
            nn.Dropout(dropout_rate),
    ...
    def forward(self, g, x_prot):
        ...
        # Adaptive pooling for molecule representation (now includes edge features)
        mol_graph_emb = self.adaptive_pool(g, h, edge_feat)
