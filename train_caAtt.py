...
train_loader = GraphDataLoader(df_train, batch_size=batch_size, shuffle=True)
del df_train
df_valid = AffinityDataset(pd.read_pickle("processed_data/valid_processed.pkl"))
valid_loader = GraphDataLoader(df_valid, batch_size=batch_size, shuffle=False)
del df_valid
df_test = AffinityDataset(pd.read_pickle("processed_data/test_processed.pkl"))
test_loader = GraphDataLoader(df_test, batch_size=batch_size, shuffle=False)
# del df_test
gc.collect()
...
Logger.current_logger().report_text(f"Hyperparameters: lr={lr}, weight_decay={weight_decay}, batch_size={batch_size}, num_epochs={num_epochs}, dropout={dropout}, h_features={h_features}, in_feat={in_feat}, prot_dim={prot_dim}, edge_feat_dim={edge_feat_dim}, patience={patience}")
...
torch.save(best_model_state, f"weights/best_model_{date}_{test_r2}.pth")
